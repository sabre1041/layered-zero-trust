...
The _qtodo_ Quarkus application runs in this namespace. The PostgreSQL database (`qtodo-db`) runs in the separate `qtodo-db` namespace so that the database credential Secret is not co-located with the application.

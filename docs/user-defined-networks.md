...
Requires `spec.useMultiNetworkPolicy: true` on `network.operator.openshift.io/cluster`. If that Cluster Network Operator setting cannot be enabled, set `udn.networkPolicy.enabled` and `app.udn.networkPolicy.enabled` to `false`. UDN membership plus cluster-network `NetworkPolicy` still isolate PostgreSQL.

## Enabling UDN

### Prerequisite: MultiNetworkPolicy

If you keep UDN network policies enabled (the default):
...
Wait until `network.operator.openshift.io/cluster` reports that the change has been reconciled before installing the feature.
...
| `udn.networkPolicy.enabled` | both | Create MultiNetworkPolicy objects | `true` |
